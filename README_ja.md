...
| 配送失敗の可視化 | 異常な配送結果は、カウンタと shutdown summary として見える形に残します。`UnexplainedLost` も明示的な状態として残すため、異常な実行結果を「ログファイルが短いだけ」として見落としにくくします。 |
...
ベンチマークには、sink unavailable、burst backpressure、worker crash、mixed worker behavior、shutdown behavior などのマルチプロセス resilience profile も含まれます。これらはスループット主張ではなく、試みられたレコードを delivered / known-rejected / known-dropped / unexplained-lost として説明できるかを見るものです。
