...
dsafelogger/
  ...
  _levels.py           # カスタムログレベル登録・管理（register_custom_level, get_all_level_map 等）
...
| 変数 | 用途 | 推奨型 |
|------|------|--------|
| `_lifecycle_lock` | Configure / GetLogger / RegisterLevel / shutdown の状態遷移保護 | `threading.RLock` |
...
_levels.py（ユーティリティモジュール）
  ├── register_custom_level(name, value, abbreviation, color)
...
2. 引数バリデーション
   ...
   ├── console_out は exact True / exact False / exact "only" のみを許可
...
6. マージ済み設定の最終バリデーション
   ...
   ├── console-only では file sink 専用設定を `ValueError`
   └── `env_prefix` 自体は INI / config_dict / 環境変数から変更させない

7. Fail-Fast パーミッション検証（file-enabled の場合のみ）
   ├── `log_path`
...
class IniLoader:
    ...
    # 型変換ルール
    BOOL_KEYS = frozenset({'is_async', 'archive_mode', 'structured', 'enable_hash', 'sens_kws_replace'})  # v16: sens_kws_replace 追加
    CONSOLE_OUT_KEY = 'console_out'  # v23m: bool alias + "only" を専用 parser で処理
...
class EnvParser:
    ...
    def parse_bool_env(env_value: str | None) -> bool | None:
        """{prefix}_COLOR / {prefix}_HASH 用の真偽値パーサ。
    ...
    @staticmethod
    def parse_console_env(env_value: str | None) -> bool | Literal["only"] | None:
        """{prefix}_CONSOLE 用の出力 mode パーサ。
        "1"/"true" → True, "0"/"false" → False, "only" → "only"。
        その他の非空値は、file sink 作成有無を曖昧にしないため ValueError。"""
        if env_value is None:
            return None
        v = env_value.strip().lower()
        if not v:
        ...
        if v == 'only':
            return 'only'
        raise ValueError('Invalid console env value')
...
`_levels.py` はカスタムログレベルの登録情報を一元管理するモジュールである。public API の `dsafelogger.RegisterLevel()` は `__init__.py` 側で lifecycle guard を担当し、`_levels.py` の内部実装 `register_custom_level()` に委譲する。`_levels.py` は各コンポーネントが統合マップを取得するためのクエリ関数群も提供する。
...
### 14.3. `register_custom_level` 内部関数
...
def register_custom_level(
    ...
    # __init__.py の public RegisterLevel() が _lifecycle_lock と
    # ConfigureLogger 呼び出し順序の guard を担当する。
        ...
        for existing_name, (
            ...
            if value == existing_value:
                if (
                    ...
                    return  # spawn 再 import 対応。same-definition 再登録は no-op
                raise ValueError(
            ...
            if name_upper == existing_name:
                if (
                    ...
                    return  # same-definition 再登録
                raise ValueError(
                    f'level {name_upper!r} is already registered'
                )
...
`__init__.py` 側の public `RegisterLevel()` は次の lifecycle guard を担当する:
...
> v20: public `RegisterLevel()` が `_lifecycle_lock` で lifecycle guard を行い、内部 `register_custom_level()` が `_levels_lock` で `_custom_levels` 更新を保護する。これにより free-threaded Python (3.13+) での並行呼び出しによる破損を防止する。

- `_configure_state == 'shutting_down'` も上記の拒否条件に含む。終了処理中の追加登録は shared state の整合を壊しうるため認めない。
- `spawn` による worker 再 import を考慮し、**同一定義**（name/value/abbreviation/color が一致）の再登録は no-op とする。一方、不一致再登録は registry divergence とみなし `ValueError` とする。
...
def install_convenience_methods(logger_class: type) -> None:
    ...
    例: RegisterLevel('TRACE', 5, 'TRC') → logger.trace(msg, *args, **kwargs)
    """
...
`ConfigureLogger()` 後の `RegisterLevel()` は `RuntimeError`。この制約により、Formatter / Handler / バリデーションが初期化後に不整合を起こすことを構造的に防止する。
...
| `RegisterLevel()` が `ConfigureLogger` 後に呼ばれた | `RuntimeError` を発生 |
| `RegisterLevel()` でビルトイン値/名前/略称の上書き | `ValueError` を発生 |
| `RegisterLevel()` で略称が3文字でない | `ValueError` を発生 |
| `RegisterLevel()` の same-definition 再登録 | no-op |
| `RegisterLevel()` の不一致再登録 | `RuntimeError` を発生 |
...
| `{prefix}_CONSOLE` | `"1"/"true"/"0"/"false"/"only"` (大文字小文字不問。ただし `yes` / `no` / `on` / `off` は invalid) | `console_out` |
...
| v23m | 2026-05-23 | multiprocess observability を正式化。`_runtime_warning.py` と `_shutdown_report.py` を追加し、`mp.ConfigureLogger(runtime_warning_path=..., shutdown_report_path=...)`、`mp.GetDeliveryStatus()`、`mp.DeliveryStatus` を公開した。WriterRuntime は accepted/delivered counters、worker DETACH local drop summary、3 種 breakdown、shutdown report atomic write、warning queue drain marker を保持する。runtime STATUS は live worker を crash と誤認しないため `missing_detach_clients` を 0 とし、shutdown report 側で active ledger から missing client IDs/PIDs を出力する。MP runtime wire protocol は同一バージョン前提で、ATTACH pid と DETACH local drop summary を追加する。 |

---

## v23m Runtime Warning Design Integration
...
## v23m Console-only Implementation Notes

The single-process configuration pipeline resolves `console_out` into an internal `OutputMode(console_enabled, file_enabled)`. `PipelineBuilder` constructs root file handlers and module file transports only when `file_enabled=True`. When `file_enabled=False`, the root transport contains only `ColorStreamHandler(stderr)` or the async listener-side equivalent. File formatter construction is scoped to file-sink branches so console-only cannot reference an uninitialized file formatter.

Validation is split into common validation and file-sink validation. Common validation covers strict `console_out` argument validation, global/module level validation, structured formatter conflicts, and generic numeric range checks. File-sink validation covers routing, hash, manifest, retention, and writable path checks. Console-only runs common validation and then `validate_console_only_conflicts()` over the final merged global and module config.

The environment parser uses a dedicated `parse_console_env()` for `{prefix}_CONSOLE` so invalid values fail fast. Generic boolean env parsing for color/hash keeps its existing compatibility behavior. INI/config_dict parsing uses a dedicated `console_out` parser that preserves existing bool aliases and adds the `only` literal.

Because environment variables are the highest-priority layer, `{prefix}_CONSOLE=1` / `true` can override an API/INI `console_out="only"` request back to file + console, and `{prefix}_CONSOLE=0` / `false` can override it to file-only output. The no-file-sink guarantee is therefore tied to the final merged output mode and to `ConfigureLogger(console_out="only")` being the first explicit initialization before any `GetLogger()` auto-fire or earlier explicit configuration.
