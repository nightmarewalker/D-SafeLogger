...
4. **3行で始めて、設定で運用ポリシーを追加**：最小構成は3行です。呼び出し側のコードはそのままに、設定で9種類のルーティング戦略（`daily`, `hourly`, `size` など）、JSON Lines、SHA-256 sidecar / manifest、機微情報マスキング、診断モード、コード / INI・dict / 環境変数による設定レイヤーを追加できます。

5. **堅牢かつ柔軟なマルチプロセス logging**：親プロセス側 Writer がファイル出力を集約し、worker は IPC 経由でレコードを送ります。Process / Pool / ProcessPoolExecutor の各構成に対応し、配送結果はサイレントにせず分類します。
...
| 運用時の制御 | 運用時の上書きは、明示的で operator-owned な操作として設計されています。ログレベル、ルーティング、ハッシュ出力、timeout 挙動は rebuild なしで切り替えられます。一方、ローカル変数を展開する診断モードは環境変数による明示的な opt-in に限定され、由来不明の INI ファイルからは有効化できません。 |
...
**配送状態の分類記録** とは、レコードごとに `KnownRejected` / `KnownDropped` / `UnexplainedLost` と `partial_delivered` を分類し、`mp.GetDeliveryStatus()`、runtime warning JSON Lines、shutdown report JSON に反映する仕組みです。詳細は [`examples/12_multiprocess_logging.md`](examples/12_multiprocess_logging.md)、[`docs/design/v23k_supplements/delivery_status_schema.md`](docs/design/v23k_supplements/delivery_status_schema.md)、[BENCHMARK.md](BENCHMARK.md) を参照してください。
...
INI ファイル、dict 設定、モジュール別ルーティング、優先順位は [Configuration Guide](examples/02_configuration_guide.md) を参照してください。ルーティングモードの選び方、削除 / アーカイブによる保持、長期稼働時のファイルライフサイクル例は [Long-Running Service](examples/07_long_running_service.md) を参照してください。
...
- [アーキテクチャ分析ホワイトペーパー](docs/design/D-SafeLogger_v23k_WhitePaper.md)
- [基本設計仕様書](docs/design/D_SafeLogger_Specification_v23k_full.md)
