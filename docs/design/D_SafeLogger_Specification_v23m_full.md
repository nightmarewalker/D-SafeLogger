# D-SafeLogger 基本設計仕様書 v23m (Capture/Transport/Sink 3層化・dsafelogger.mp 正式仕様・外部 log rotation 共存・Control Plane/Backpressure 固定化・Vendor-Agnostic)
...
* **カスタムログレベル (Custom Log Levels):**
  `RegisterLevel()` により、標準5段階（DEBUG/INFO/WARNING/ERROR/CRITICAL）に加えて任意の数値位置にカスタムログレベルを差し込み可能。3文字略称・ANSIカラー・便利メソッドの一括登録を `ConfigureLogger` 前の単一呼び出しで完結させる。ビルトイン5段階は不可侵として保護され、3層設定管理パイプラインとも完全に整合する。
...
| `{prefix}_CONSOLE` | コンソール/file 出力 mode の強制制御 | `"1"/"true"`（file + console）, `"0"/"false"`（file only）, `"only"`（console only） | INI `console_out`、引数 `console_out` |
...
- 有効値: `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`（大文字小文字不問）に加え、`RegisterLevel()` で登録済みのカスタムレベル名も使用可能
...
- レベル名にはビルトイン5段階に加え、`RegisterLevel()` で登録済みのカスタムレベル名も使用可能
...
環境変数 `{prefix}_CONSOLE`（有効値: `"1"` / `"true"`、`"0"` / `"false"`、`"only"`: 大文字小文字不問）により、file + console、file only、console only の出力 mode を上書き制御する。
D エコシステム（ecosystem）の他のパラメータと方向性を揃えるための仕様であり、ソースコード内で `console_out=False` と設計されているバックグラウンドサービスでも、起動前の環境変数設定により開発時のみ file + console へ上書きして起動できる。`D_LOG_CONSOLE=only` は single-process の console-only mode を選択し、`yes` / `no` / `on` / `off` などの旧 bool alias は file sink 作成有無を曖昧にしないため `ValueError` とする。
...
| `console_out` | `console_out` | bool or `"only"` | `true`/`false` に加え `1`/`0`/`yes`/`no`/`on`/`off` と `only` を許容。`only` は single-process の console-only mode を表す |
...
| `color_{略称}` | — | str | ANSI SGR パラメータの数値部分（例: `36`, `1;31`, `38;5;208`）。`color_dbg`, `color_inf`, `color_war`, `color_err`, `color_cri` がビルトイン対応。`RegisterLevel()` で登録されたカスタムレベルの略称も使用可能。ConfigureLogger 引数・環境変数からの設定は不可（第2層専用）。空文字列で該当レベルのカラー化を無効にする |
...
**未知キーの扱い**: `[global]` セクションに未知のキーが記載されていた場合、そのキーは stderr に警告出力した上で無視する。既存の有効キーに対する型変換エラーとは異なり、未知キーは設定ミスの通知対象ではあるが、ただちに起動停止にはしない。ただし、`color_` プレフィックスを持つキーはパターンベースで認識されるため、固定キー一覧には含まれない。`color_` プレフィックスのキーは略称部分がビルトイン5段階または `RegisterLevel()` で登録済みのカスタムレベルの略称と一致するかを動的に検証し、未知略称の場合は stderr に警告出力してスキップする（Fail-Fast ではない）。
...
**カスタムレベル名のバリデーション**: `default_level` およびモジュール別セクションの `level` キーは、ビルトイン5段階に加え `RegisterLevel()` で登録済みのカスタムレベル名も受け付ける。未登録のレベル名が指定された場合は `ValueError` を送出する（Fail-Fast）。

**v23m: merge 後の統一検証**: Python API 引数、INI/config_dict、環境変数を merge した後、最終的な設定に対して同じ検証を再実行する。`structured=True` と `fmt` / `file_fmt` / `console_fmt` の同時指定、未登録 `default_level`、`backup_count < 0`、`max_count < 1`、`suffix_digits < 1`、`startup_interval` の `interval < 1` は、指定元に関係なく `ValueError` とする。Python API 直指定の bool 引数（`is_async`, `archive_mode`, `structured`, `enable_hash`, `sens_kws_replace`）は `bool` 型のみを受け付け、文字列の truthy/falsy 解釈は行わない。`console_out` は exact `True` / exact `False` / exact `"only"` のみを受け付け、`1` / `0` / `"true"` / `"ONLY"` などは `TypeError` とする。console-only の final config では file sink 専用設定を `ValueError` とし、file-enabled config では従来どおり file sink 検証を行う。
...
* **5状態中の競合時挙動**:
  ...
  * `shutting_down` 中の `RegisterLevel()` は `RuntimeError` とする。
...
> ※ ただし、`RegisterLevel()` 内部では `logging.addLevelName(value, name)` を呼び出し、標準 `logging` モジュールにカスタムレベルの数値→名前マッピングを登録する。これは `logger.log(value, msg)` や `isEnabledFor(value)` など、標準APIの正常動作に必要な登録であり、略称変換（`INFO` → `INF`）とは異なる。

### 9.9. カスタムログレベル (Custom Log Levels) 仕様

`RegisterLevel()` 関数により、標準の5段階（DEBUG/INFO/WARNING/ERROR/CRITICAL）に加えて任意の数値位置にカスタムログレベルを差し込み可能とする。本機能はオプトイン（`RegisterLevel` を呼ばなければ一切の影響なし）であり、使用しない場合は v14.5 と完全に同一の動作を維持する。
...
`ConfigureLogger()` 後の `RegisterLevel()` は `RuntimeError` を送出する。この制約により、Formatter / Handler / バリデーションが初期化後に不整合を起こすことを構造的に防止する。
...
`RegisterLevel()` で登録されたカスタムレベル名は、3層パイプラインの全ての層で使用可能になる:
...
コード上で `RegisterLevel('TRACE', ...)` を呼ばずに、INIファイルや環境変数で `TRACE` を指定した場合は、有効なレベル名の集合に含まれないため、既存の Fail-Fast バリデーションにより `ValueError` が送出される。

#### 9.9.5. 便利メソッドの動的生成

`RegisterLevel('TRACE', value=5, ...)` を呼ぶと、`ConfigureLogger` の初期化処理において `DSafeLogger` クラスに `logger.trace(msg, *args, **kwargs)` メソッドが動的に追加される。
...
公開 API の関数名は、`ConfigureLogger()` / `GetLogger()` / `RegisterLevel()` / `ReopenLogFiles()` / `SafeShutdown()` のように PascalCase を採用する。これは通常の Python 関数名に対する PEP 8 の snake_case 方針からは意図的に外れる設計判断であり、既存 public API との一貫性と「D-SafeLogger の操作 API」であることの視認性を優先するための例外である。
...
* **v15 (カスタムログレベル & ファイル完全性検証)**:
  * `RegisterLevel()` 関数の追加（§9.9, §10.3）。任意の数値位置にカスタムログレベルを差し込み可能。3文字略称・ANSIカラー・便利メソッドの一括登録を、`ConfigureLogger` 前の単一呼び出しで完結。ビルトイン5段階（DEBUG/INFO/WARNING/ERROR/CRITICAL）は不可侵として保護。3層設定管理パイプラインとの完全整合: 登録済みカスタムレベルは環境変数・INI・引数の全てで使用可能。
...
* **v16 (センシティブキーワードのカスタマイズ & 辞書ベース設定)**:
  ...
  * §11（後方互換性への影響）を削除。
* **v17 (コンソールカラーパレット設定)**:
  * INI ファイルおよび config_dict の `[global]` セクションに `color_{略称}` キーを追加（§5.2, §5.3）。ビルトイン5段階（DBG/INF/WAR/ERR/CRI）のコンソールカラーを、ターミナル環境や視覚特性に応じて変更可能に。`RegisterLevel()` で登録されたカスタムレベルのカラーも同一の命名規則で上書き可能。
  ...
  * カラーパレットのマージ順序: ビルトインデフォルト → `RegisterLevel()` 指定カラー → INI/辞書の `color_{略称}` キー。
...
* **v20 (Capture/Transport/Sink 3層化・Vendor-Agnostic・FrozenContext)**:
  * 内部アーキテクチャを Capture / Transport / Sink の3層モデルに再構成（§11.2）。公開 API（`ConfigureLogger` / `GetLogger` / `RegisterLevel`）に変更なし。
...
* **v20 (レビュー指摘反映・堅牢性強化)**:
  ...
  * `RegisterLevel()` 全体を `_lifecycle_lock` で保護し、free-threaded Python での並行呼び出しによる `_custom_levels` 破損を防止。
...
* **v22f (registry / payload 構築原則 / Writer crash 補強)**:
  * registry hash 照合タイミング、spawn 再 import 時の `RegisterLevel()` 冪等再登録ルールを追加。
...
v23m では単一プロセス API に `ConfigureLogger(console_out="only")` を追加する。これは root / module file sink を構築せず、既存の stdlib logging integration、formatter、context、console color、async transport を維持したまま stderr へ出力する CLI / local development 向け profile である。

`console_out` の意味は `True` が file + console、`False` が file only、`"only"` が console only である。Python API では exact `True` / exact `False` / exact `"only"` のみを許可し、`1`、`0`、`None`、`"true"`、`"ONLY"` は `TypeError` とする。INI/config_dict は既存 bool aliases に加えて `console_out = only` を許可する。`D_LOG_CONSOLE` は `1` / `true` / `0` / `false` / `only` のみを許可し、それ以外は `ValueError` とする。`D_LOG_CONSOLE=1` / `true` は API/INI の `console_out="only"` を file + console へ、`D_LOG_CONSOLE=0` / `false` は file only へ上書きできるため、operator は環境変数による意図反転を明示的に扱う必要がある。

console-only は file-oriented settings を黙って無効化する emergency switch ではない。3 層 merge 後の final config に `log_path != "."`、`pg_name != "Default"`、`file_fmt`、routing、retention、hash、manifest、module path など file sink 専用設定が残る場合は `console-only` を含む `ValueError` で fail-fast する。`pg_name` の判定は final merged effective value と `"Default"` の生比較であり、post-merge 正規化は導入しない。

routing threshold validation は file-sink 専用 validation とし、merge 前の Python API 基本検証では実行しない。file-enabled configuration では従来どおり merge 後の file-sink validation が `routing_mode="size"` / `"count"` の threshold を検証する。console-only では file-sink validation と path writable validation を実行せず、console-only conflict validation が唯一の file-oriented conflict gate になる。

`ReopenLogFiles()` は file lifecycle API のままであり、console-only では reopen 対象 file sink がないため `RuntimeError` を送出する。file 非作成保証は、最終 merge 後の mode が console-only であり、かつ `ConfigureLogger(console_out="only")` が `GetLogger()` auto-fire や他の explicit 初期化より前の最初の explicit 初期化である場合に限られる。`SafeShutdown()` は console-only でも有効で、async console-only では queue drain と listener stop を行う。`dsafelogger.mp` は Writer-owned file sinks と delivery-state accounting を中核価値とするため、`console_out="only"` および `D_LOG_CONSOLE=only` を拒否する。`{prefix}_CONSOLE` の invalid 値は file sink 作成有無を左右するため fail-fast するが、`{prefix}_COLOR` / `{prefix}_HASH` の invalid 値は後方互換のため従来どおり override なしとして扱う。
